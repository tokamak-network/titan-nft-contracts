...
describe('L2ERC721Bridge', () => {
    ...
    describe('# L2ERC721BridgeProxy ', () => {
        ...
        it('upgradeTo onlyAdmin', async () => {
            ...
            await expect(
                deployed.l2ERC721BridgeProxy.connect(addr1).upgradeTo(
                    ethers.constants.AddressZero))
                .to.be.rejectedWith("not admin");

            await (await deployed.l2ERC721BridgeProxy.connect(deployer).upgradeTo(
        ...
        it('upgradeToAndCall onlyAdmin', async () => {
            ...
            await expect(
                deployed.l2ERC721BridgeProxy.connect(addr1).upgradeToAndCall(
        ...
        it('changeAdmin onlyAdmin', async () => {
            await expect(
                deployed.l2ERC721BridgeProxy.connect(addr1).changeAdmin(
                    addr1.address
                ))
                .to.be.rejectedWith("not admin");

            await deployed.l2ERC721BridgeProxy.connect(deployer).changeAdmin(addr1.address)
            expect(await deployed.l2ERC721BridgeProxy.admin()).to.be.eq(addr1.address)

            await deployed.l2ERC721BridgeProxy.connect(addr1).changeAdmin(deployer.address)
    ...
    describe('# L2ERC721Bridge ', () => {
        ...
        it('initialize only Owner', async () => {
            ...
            await expect(
                deployed.l2ERC721Bridge2.connect(addr1).initialize(
                    deployed.l2MessengerAddress,
                    deployed.l1ERC721BridgeProxy.address
                ))
                .to.be.rejectedWith("not owner");

            await deployed.l2ERC721Bridge2.connect(deployer).initialize(
                ethers.constants.AddressZero,
                deployed.l1ERC721BridgeProxy.address
            )

            expect(await deployed.l2ERC721BridgeProxy.MESSENGER()).to.be.eq(ethers.constants.AddressZero)
            expect(await deployed.l2ERC721BridgeProxy.OTHER_BRIDGE()).to.be.eq(deployed.l1ERC721BridgeProxy.address)
        ...
        it('setSemver only Owner', async () => {
            await expect(
                deployed.l2ERC721Bridge2.connect(addr1).setSemver(
                    1,1,1
                ))
                .to.be.rejectedWith("not owner");

            await deployed.l2ERC721Bridge2.connect(deployer).setSemver(
                1,2,3
            )
            expect(await deployed.l2ERC721Bridge2.MAJOR_VERSION()).to.be.eq(1)
            expect(await deployed.l2ERC721Bridge2.MINOR_VERSION()).to.be.eq(2)
            expect(await deployed.l2ERC721Bridge2.PATCH_VERSION()).to.be.eq(3)
        });
    });
